...
class PIDController:
    def __init__(self, kp, ki, kd, friction=0.05, noise_level=0.005, delay=0.001, max_acceleration=1.0):
        self.kp = kp
        self.ki = ki
        self.kd = kd
...
def simulate_pid(kp, ki, kd, sim_time=st, dt=0.01, setpoint=setpoint, max_track_width=max_track_width):
    ...
    # Sensor filtering (simulate sensor lag)
    ...
    for i in range(n_steps):
        # Sensor filtering: update filtered position from true position.
        ...
        # Use the control as an acceleration command.
        ...
        # Reflective boundaries: if new_position exceeds limits, reflect and damp velocity.
...
# Now, perform a grid search where all three PID constants take on the same set of values.
