...
def calculate_settling_time(time_array, position_array, setpoint, tolerance=0.05, window_duration=2.5):
...
class PIDController:
    def __init__(self, kp, ki, kd, friction=0.05, noise_level=0.005, delay=0.001, max_acceleration=1.1):
        self.kp = kp
        self.ki = ki
        self.kd = kd
...
def simulate_pid(kp, ki, kd, sim_time=st, dt=0.01, setpoint=setpoint, max_track_width=max_track_width):
    ...
    pid = PIDController(kp, ki, kd, friction=0.06, noise_level=0.005, delay=0.002, max_acceleration=1.2)
    pid.reset()
    
    positions = []
    
    # Nonlinear friction parameters
    viscous_friction = 0.15   # proportional to velocity
    coulomb_friction = 0.06  # constant opposing force
